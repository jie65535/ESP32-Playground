...
void BreakoutScoreService::begin(Stream& output) {
    Preferences preferences;
    if (!preferences.begin(NAMESPACE, false)) {
        output.println(F("[breakout] score storage unavailable"));
        return;
    }
...
uint16_t BreakoutScoreService::bestScore() const {
    return scores_[0];
}

uint16_t BreakoutScoreService::scoreAt(uint8_t index) const {
    return index < SCORE_COUNT ? scores_[index] : 0;
}

void BreakoutScoreService::recordScore(uint16_t score) {
