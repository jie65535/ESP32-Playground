...
当前先采用“小扬声器校准曲线”：`1% = -45 dB`、`10% = -40 dB`、`20% = -32 dB`、`30% = -25 dB`、`40% = -22 dB`、`50% = -19 dB`、`60% = -17 dB`、`100% = 0 dB`，区间线性插值，0% 单独硬静音。它保留手机曲线的顶部形状，同时抬高低端，避免这块未知扬声器在 30% 以下落入听感/噪声底。该曲线仍是实验性校准，后续以实际试听为准。
