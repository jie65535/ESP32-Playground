...
参数真正改善生成质量，以及除故事续写以外还能形成哪些本地应用。本记录区分已经
完成的 P96/F512 实验和仍停留在计算阶段的更大候选，不把预测写成硬件结论。

结论先行：

- 在不修改分区的前提下，P96/F512、7.56M 参数模型导出为 3,917,660 bytes，
  4MiB model 分区仍余 276,644 bytes；真机两轮为约 5.1--5.2 tok/s。
- 同一 4096-token 验证窗口上，新 INT4/int8 模型 PPL 11.14，旧模型为 13.31；
  改进明确，但真机样例仍有角色、对象和因果漂移。
- 保留当前 4MiB PGOS App 和 3.875MiB SPIFFS，仅回收第二个 OTA 槽，可把
  `model` 从 4MiB 扩到 8MiB。
- 沿用首轮规划的 `V=8192, D=128, L=6, H=4, F=415, S=512` 参数族时，8MiB 分区的数学
...
- 新模型训练 12,000 steps、采样 196.6M tokens，最终 validation loss 2.3655、
  PPL 10.65；继续扩容前应先积累固定提示词和更多 seed 的文本质量证据。
...
| 总参数量 | 7,563,744 |
| INT4 `model.bin` | 3,917,660 bytes（3.74MiB） |
| 生成动态 PSRAM | 4,277,564 bytes（约 4.08MiB，真机差值） |
| 生成中 PSRAM 空闲 | 3,669,732 bytes（约 3.50MiB） |
| PGOS 端到端吞吐 | 5.09--5.2 tok/s |
| 训练量 | 12,000 steps，196.6M sampled tokens |
| 训练数据 | 112,698,516 train tokens + 566,324 val tokens |
| 最终验证指标 | loss 2.3655，perplexity 10.65，末段仍缓慢下降 |

### 首轮扩容实验：P96/F512

本轮目标不是压满 4MiB 分区，而是在现有系统预算内同时小幅增加 PLE 和 dense FFN。
checkpoint 为：

```text
src/third_party/esp32_ai/runs/ple-zh-p96-f512-12k-s0.pt
```

训练曲线从 step 0 的 val 9.0434 持续下降到 step 11999 的 2.3655，没有出现训练/验证
明显分叉。导出后做了以下固定窗口验证：

| 模型/路径 | CE | PPL | 说明 |
|---|---:|---:|---|
| 新 checkpoint FP32 | 2.2874 | 9.85 | 同一 4096-token 顺序窗口 |
| 新模型 group-128 INT4/fp32 activation | 2.4097 | 11.13 | 相对 FP32 CE +0.1223 |
| 新模型 group-128 INT4/int8 activation | 2.4108 | 11.14 | 与真机输出头路径一致 |
| 旧模型 group-128 INT4/int8 activation | 2.5885 | 13.31 | 旧 P64/F415 基线 |

量化组比较结果为：

| group | 模型大小 | PPL | 4MiB 分区余量 | 结论 |
|---:|---:|---:|---:|---|
| 128 | 3,917,660 | 11.13 | 276,644 | 当前选择；沿用已验证布局 |
| 64 | 4,027,996 | 10.97 | 166,308 | 收益小、工程余量更紧 |
| 32 | 4,263,516 | 10.69 | -69,212 | 超出当前分区 |

C/PyTorch golden 的 top token 均为 271，max abs diff 0.00002、RMS diff
0.000003，数值验证 PASS。真机第一轮完成于 85 token、约 5.1 tok/s；第二轮完成于
77 token、日志为 5.09 tok/s。生成和退出的 PSRAM 行为与 028 记录一致。

固定窗口指标证明新模型明显优于旧模型，但两轮真机文本仍出现角色、物体和因果
漂移。当前结论是“值得替换旧模型”，不是“故事一致性问题已解决”。后续评价应增加
固定 seed、多提示词批量样本和重复率/实体一致性统计，避免只凭两段随机输出判断。

## 不做 OTA 时的 Flash 上限

当前分区为两个 4MiB App 槽、4MiB model、3.875MiB SPIFFS 和 64KiB coredump。
如果后续确定不需要 OTA，可改成单个 factory App。下表是首轮规划时按 INT4
group-128 布局和固定 `F=415` 做的容量边界计算，用于说明 Flash 上限；它不是当前
P96/F512 模型族的直接外推：
...
PSRAM 空闲以当前约 7,947,296-byte 空闲基线和分配器开销估算，必须再由真机
采样确认。

| 方案 | 配置变化 | 参数量 | 模型 | 预计生成中 PSRAM 空闲 | 取舍 |
|---|---|---:|---:|---:|---|
| 当前基线 | P96, F512 | 7.56M | 3.74MiB | 3.50MiB（实测） | 已烧录，约 5.1--5.2 tok/s |
...
P96/F512 已完成 12,000 steps，validation 到末段仍在缓慢下降。下一步不直接训练
最大模型，推荐顺序调整为：

1. 为当前 P96/F512 补固定 seed、多提示词批量生成和实体一致性统计，建立文本基线。
2. 先单独比较 top-k/top-p、repetition penalty，确认无需训练的采样收益。
3. 若当前模型仍受容量限制，再训练 P192/F512，建议 12,000--16,000 steps，并与
   P96 使用同一验证集、固定窗口和 prompts。
4. 若 P192 在 INT4 和真机输出上有稳定收益，再训练 P256/F415 或 P256/F512。
5. 最后才考虑 V16384；它需要重新训练 tokenizer、重新编码全部数据、生成新
   `vocab.h` 和提示词 token ID，实验成本和运行时变化都更大。
6. 每个胜出候选至少补一个 seed，避免把随机波动当作结构收益。
...
本轮已完成 P96/F512 模型的训练、group-128 导出、双 OTA 槽兼容固件烧录和短时真机
验证；取消 OTA、8MiB 模型分区、P192/P256、更换数据领域和长期稳定性仍留待后续决定。
