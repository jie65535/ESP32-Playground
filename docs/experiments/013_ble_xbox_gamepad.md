...
1. 启动时开放一次 60 秒的新设备配对窗口。
2. 连接成功后立即调用 `enableNewBluetoothConnections(false)` 停止发现扫描。
3. 60 秒内未连接也会停止扫描，不会永远保持射频扫描。
4. 已绑定手柄在扫描关闭时仍可按 Bluepad32 语义发起重连；要接受新手柄时由页面或 USB 命令显式再开一个 60 秒窗口。

Bluepad32 当前 BLE 扫描参数的 interval/window 都是 48 个 0.625 ms 单位，即扫描开启期间约为 30 ms / 30 ms 的连续窗口。把它限制为显式的 60 秒配对期，比未连接时永久扫描更适合 PGOS 的常驻系统定位。
...
1. 把空闲时间临时设为 5 分钟，完整等待一次自动断开并确认 Xbox 手柄随后进入自身省电状态。
2. 扫描关闭后关闭再打开已绑定手柄，复测无需重新开放配对窗口的自动重连。
