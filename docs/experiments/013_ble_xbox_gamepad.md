...
- 当前构建资源：RAM 77208 / 327680 bytes，Flash 1726465 / 6553600 bytes。
...
- 2026-07-20 二次修正：上一版迟滞仍以绝对零点判断，摇杆从推动位置回到带偏移的中心值时可能无法退出 engaged 状态，导致每个 HID 报告继续刷新 `lastActivityMs`。现改为用连接后的首个完整报告分别记录四轴和两扳机中心基线，死区和迟滞均相对基线计算；主机回归测试覆盖“偏移中心 -> 推动 -> 带偏移回中 -> 稳定空闲”。修正版已烧录 COM3 并确认启动，因现场无法连接手柄，完整 5 分钟断开仍待实测。
- 2026-07-25 代码复核：手柄活动时间现在由 `BleGamepadService` 以一次性活动信号交给
  `SystemKernel`，连续摇杆/扳机/按住输入会同时刷新息屏计时；连接事件也会自动唤醒背光。
  `gamepad status` 增加 `idle_remaining_ms` 与 `disconnect_pending`，Display 状态增加
  `idle_age_ms`，用于区分活动误刷新、超时请求和 Bluepad32 异步断开回调延迟。相同设为
  5 分钟时，只有最后活动都来自手柄才会严格同点；USB/TCP 活动仍只刷新息屏，不延长手柄
  空闲断开，这是两个不同的策略边界。修复后的 5 分钟真机长测待记录。

2026-07-25，修正版烧录 COM3 后实测：Xbox 自动连接期间设备未复位；连接事件可唤醒
背光。停止操作时 `input_age_ms` 持续增长；轻推一次摇杆后，状态从约 72 秒空闲重置为
`input_age_ms=8948`、`idle_remaining_ms=291052`，数据包从 50 增至 256，确认连续
手柄活动会重置共享息屏/手柄活动时间。完整 5 分钟自动断开仍待长测。

2026-08-02，Xbox `045E:0B13` 连接期间同时开启 Wi-Fi STA、PGOS Studio 控制和
TCP 19002 完整帧镜像。14 秒短测持续收帧，设备无复位，镜像发送无停滞，控制 RTT
约 149.9 ms；这只覆盖基础共存冒烟，不替代输入延迟、丢包和长时间射频竞争测试。

## 后续上板验证清单

1. 连接手柄后先用摇杆操作并回中，再把空闲时间设为 5 分钟；确认 `input_age_ms` 持续增长，达到约 300000 ms 后手柄关机并进入 30 秒重连等待。
2. 在 30 秒等待、重连扫描和暂停三种阶段分别关闭再打开已绑定手柄：主动断开后的首次恢复应不晚于剩余 30 秒等待，进入周期窗口后应不晚于 20 秒暂停结束。
3. 延长 BLE 与 Wi-Fi STA + PGOS Studio 镜像共存测试，记录手柄输入延迟、丢包、主循环 duty 和不同 RSSI 下的镜像 FPS。
