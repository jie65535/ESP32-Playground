...
- NTP 状态显示留待入网稳定后单独增加。
- 目标服务器地址保存在 NVS；设备端远程控制页可用软键盘编辑地址、启停连接和立即重连，`server set <host> <port>` 保留为 USB 维护入口。Wi-Fi 或上位机恢复时退避重连。
- PGOS Studio 直接监听控制 TCP，提供方向/确认/返回/Home、结构化状态与有界日志，不耦合具体设备页面。
- 独立 TCP 19002 已实现 320×240 RGB565 屏幕镜像 MVP；镜像由上位机临时控制，不写入设备设置。完整帧发送已改为有界非阻塞 socket 写入，Wi-Fi + Xbox BLE 共存短测可持续收帧，实际速率受完整帧带宽限制约为 0.5 FPS。
