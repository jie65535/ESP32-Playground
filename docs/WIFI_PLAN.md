...
当前已加入第一版 `ServerService`：服务器地址、端口和启用状态保存在 `server` NVS namespace；设备主动 TCP 连接服务器，发送 `PGOS/1 HELLO` 和周期 heartbeat，接收 `PING` 并回复 `PONG`。电脑端使用无依赖的 `tools/pgos_server.py`，默认监听 TCP 19000。当前仍是诊断链路，不提供认证远程 shell。
